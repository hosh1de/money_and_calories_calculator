...
class TestRecord:
    ...
    def test_init(self, kwargs, msg_err):
        assert hasattr(Program, 'Record'), msg_err('add_class', 'Record')
        result = Program.Record(**kwargs)
...
class TestCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        assert hasattr(Program, 'Calculator'), msg_err('add_class', 'Calculator')
        result = Program.Calculator(init_limit)
...
class TestCaloriesCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        assert hasattr(Program, 'CaloriesCalculator'), \
            msg_err('add_class', 'CaloriesCalculator', child=True, parent_name='Calculator')
        result = Program.CaloriesCalculator(init_limit)
...
class TestCashCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        assert hasattr(Program, 'CashCalculator'), \
            msg_err('add_class', 'CashCalculator', child=True, parent_name='Calculator')
        result = Program.CashCalculator(init_limit)
    ...
    def test_get_today_cash_remained(self, init_limit, data_records, amount, currency, today_cash_remained, msg_err,
        ...
        result.EURO_RATE = 70
        monkeypatch.setattr(Program.CashCalculator, "EURO_RATE", 70)
        result.USD_RATE = 60
        monkeypatch.setattr(Program.CashCalculator, "USD_RATE", 60)
